...
const MessageForo = () => {
  ...
  const handleChange = (e) => {
    ...
    const createPost = async () => {
        await addDoc(msnCollectionRef, { postText, dateHour, datePost, author: { name: auth.currentUser.displayName, uid: auth.currentUser.uid, email: auth.currentUser.email, image: auth.currentUser.photoURL }
         });
         setPostText("")
      };
  ...
  const msnCollectionRef = collection(db, "msn");

  const deletePost = async (id) =>{
    setLoading(true);
    const postDoc = doc(db, "msn", id);
  ...
  useEffect(() => {
    ...
    const getPosts = async () => {
      const data = await getDocs(msnCollectionRef);
